...
namespace Photon.Realtime
{
    ...
    public class Room : RoomInfo
    {
        ...
        private Dictionary<int, List<Player>> teams = new Dictionary<int, List<Player>>();
        public Dictionary<int, List<Player>> Teams
        {
            get
            {
                return this.teams;
        ...
        public byte NumberTeamMinPlayers()
        {
        ...
        public byte IndexPlayerInTeam(Player player)
        {
            byte index = 0;
            for (byte i = 0; i < Teams[player.TeamNumber].Count; i++)
            {
                if (Teams[player.TeamNumber][i] == player) index = i;
            }
            return index;
        }

        public virtual int CountPlayersInTeam(int team) => this.Teams.ContainsKey(team) ? this.Teams[team].Count : 0;

        public byte EmptySeatsInTeam(int team) => 
            Teams.ContainsKey(team) ? (byte)(MaxPlayers / CountTeams - Teams[team].Count) : (byte)(MaxPlayers / CountTeams);

        public new byte CountTeams
        {
            get
            {
            ...
            set
            {
                if (value != this.countTeams)
                {
                    if (!this.isOffline)
                    {
                        this.LoadBalancingClient.OpSetPropertiesOfRoom(new Hashtable() { { GamePropertyKey.CountTeams, value } });
                    }
                }

                this.countTeams = value;
            }
        }
        ...
        public new byte MaxPlayers
        {
            ...
            set
            {
                if (value != this.maxPlayers)
                {
                    if (!this.isOffline)
                    {
                        Debug.Log("OpJoinRandomOrCreateRoom: " + this.maxPlayers);
                        this.LoadBalancingClient.OpSetPropertiesOfRoom(new Hashtable() { { GamePropertyKey.MaxPlayers, value } });
                        Debug.Log("EndOpJoinRandomOrCreateRoom: " + this.maxPlayers);
                    }
                }

                Debug.Log("MaxPlayers: " + this.maxPlayers);
                this.maxPlayers = value;
                Debug.Log("EndMaxPlayers: " + this.maxPlayers);
            }
        }
        ...
        protected internal virtual void RemovePlayer(Player player)
        {
            ...
            if(this.Teams.ContainsKey(player.TeamNumber) && this.Teams[player.TeamNumber].Contains(player)) 
                this.Teams[player.TeamNumber].Remove(player);
        ...
        public virtual Player StorePlayer(Player player)
        {
            ...
            int numberTeam = this.NumberTeamMinPlayers();
            player.TeamNumber = numberTeam;

            if (this.Teams.ContainsKey(numberTeam))
            {
                this.Teams[numberTeam].Add(player);
            }
            else
            {
                this.Teams[numberTeam] = new List<Player> { player };
            }
            /*
            if (this.Teams.ContainsKey(numberTeam))
            {
                this.Teams.Add(numberTeam, new List<Player> { player });
            }
            else
            {
