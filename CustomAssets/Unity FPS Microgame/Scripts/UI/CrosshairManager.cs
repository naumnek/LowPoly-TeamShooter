...
namespace Unity.FPS.UI
{
    public class CrosshairManager : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            m_WasPointingAtEnemy = m_WeaponsManager.IsPointingEnemy();
            m_WasHitEnemy = m_WeaponsManager.IsHitEnemy;
        }
        ...
        void UpdateCrosshairPointingAtEnemy(bool force)
        {
            ...
            if (m_WasHitEnemy)
            {
                m_WeaponsManager.OnHitEnemy(false);
                m_CurrentCrosshair = m_CrosshairDataHitTarget;
            ...
            if ((force || !m_WasPointingAtEnemy) && m_WeaponsManager.IsPointingEnemy() && !m_WasHitEnemy)
            {
                m_CurrentCrosshair = m_CrosshairDataTarget;
                CrosshairImage.sprite = m_CurrentCrosshair.CrosshairSprite;
                m_CrosshairRectTransform.sizeDelta = m_CurrentCrosshair.CrosshairSize * Vector2.one;
            }
            else if ((force || m_WasPointingAtEnemy || m_WasHitEnemy) && !m_WeaponsManager.IsPointingEnemy())
            {
